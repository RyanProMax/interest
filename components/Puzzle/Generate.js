...
function Generate(
	{ url, width = 800, height = 800, pixel = 8, fileList, onFinish },
	ref
) {
	...
	useEffect(() => {
			...
			for (let y = 0; y < canvas.height; y += pixel) {
				for (let x = 0; x < canvas.width; x += pixel) {
					...
					for (let i = 0; i < data.length; i += 4) {
						...
						rgbas[(y / pixel) * COLUMNS + x / pixel].push([
							data[i],
							data[i + 1],
							data[i + 2],
							data[i + 3]
		...
		const generateImg = () => {
			...
			for (let i = 0; i < urls.length; i++) {
				fabric.Image.fromURL(urls[i], img => {
					const scale =
						img.height > img.width ? pixel / img.width : pixel / img.height;
					img.set({
		...
		(async () => {
			...
			fabric.Image.fromURL(url, img => {
				const scale =
					img.height > img.width ? width / img.width : height / img.height;
				img.set({
